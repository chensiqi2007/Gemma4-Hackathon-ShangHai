# CrossReaction Gemma 4 Bioinformatics Agent

## 中文说明

### 项目定位
CrossReaction Lite Version for GDG Gemma 4 Hackathon 2026 是为赛道 A 准备的公开评审项目。它展示 Gemma 4 如何在本地或私有 OpenAI-compatible endpoint 中，把自然语言生物医学分析需求转化为可审计的公开工作流：模型先通过 Native guided tool calling 选择工作流，API 再基于公开 synthetic sample data 和公开 Docker 工具链执行分析，并由 Gemma 4 对结果进行简要解释。

本仓库提供评审所需的源码、样例数据、文档和启动脚本。评审范围限于本仓库提交内容；商业版 CrossReaction 平台的生产系统、真实客户数据、私有镜像和生产凭据不属于本次提交范围。
...
| 5 分钟以内演示视频 | `docs/DEMO_SCRIPT.md` 提供 5 分钟脚本；录制视频需随比赛平台单独上传。 |
...
| 多步规划 | Gemma 4 使用 `select_workflow` 选择公开工作流，API 继续完成 sample data inspection、execution plan grounding 和 Docker execution。 |
...
### 不包含内容
- 商业版 CrossReaction 平台的生产调度、权限、缓存、数据库或部署系统。
...
1. 构建 API 与 Web。
2. 运行 API 测试。
3. 生成干净提交包到 `.review-data/submission/CrossReaction-Gemma4-Agent`。
4. 在提交包中运行边界扫描，确认不包含 `.agentdocs`、`.env`、私有路径、私有镜像、客户数据或生产凭据。

如需手动生成提交包：
```bash
bash scripts/package-submission.sh
cd .review-data/submission/CrossReaction-Gemma4-Agent
...
### 许可与边界
本公开评审项目使用 GNU General Public License version 3 only（GPL-3.0-only）。完整许可证文本见 `LICENSE`。许可证覆盖本仓库内代码和 synthetic sample data；未包含在仓库中的生产服务、私有数据、私有镜像、凭据和未公开商业材料不属于授权范围。CrossReaction 名称、标识和品牌资产不授权作商标使用，除非是为合理说明本仓库来源所必需。

Copyright 2026 LIANG Weiyin @ CrossReaction.
...
### Project Scope
CrossReaction Lite Version for GDG Gemma 4 Hackathon 2026 is a public review project prepared for Track A. It shows how Gemma 4 running behind a local or private OpenAI-compatible endpoint can turn a natural-language biomedical analysis request into an auditable public workflow: the model selects a workflow through Native guided tool calling, the API executes public synthetic sample data with public Docker tools, and Gemma 4 summarizes the computed result.

This repository provides the source code, sample data, documentation, and startup scripts needed for review. The review scope is limited to the submitted repository contents; production CrossReaction systems, real customer data, private images, and production credentials are outside this submission.
...
| Demo video within five minutes | `docs/DEMO_SCRIPT.md` provides a five-minute script; the recorded video should be uploaded separately through the competition platform. |
...
| Multi-step planning | Gemma 4 calls `select_workflow`; the API performs sample-data inspection, public execution-plan grounding, and Docker execution. |
...
### Not Included
- Production orchestration, permission, cache, database, or deployment systems from the commercial CrossReaction platform.
...
2. Runs API tests.
3. Creates a clean submission package under `.review-data/submission/CrossReaction-Gemma4-Agent`.
